    ...
    if not frame.auras then
        ...
        for i = 1, 16 do
            local button = CreateFrame("Button",
                "Luna" .. firstToUpper(frame.trueunit or frame:GetName()) .. "DebuffFrame" .. i, frame.auras)
            button:SetScript("OnEnter", showTooltip)
            button:SetScript("OnLeave", hideTooltip)
            if isPlayer then
                button.cooldown = CreateFrame("Model", button:GetName() .. "CD", button, "CooldownFrameTemplate")
...
local function WTimerUpdate()
    ...
    if this.auraID == 16 and mainEnchant then
        ...
        if this.cooldown:IsVisible() then
            CooldownFrame_SetTimer(this.cooldown, GetTime() - (LunaBuffDB[mainEnchant] - mainHandExpiration),
                LunaBuffDB[mainEnchant], 1)
        end
        if mainHandExpiration > 9 then
            this.timeFontstring:SetFont(defaultFont, LunaUF.db.profile.units["player"].auras.timertextsmallsize,
                "OUTLINE")
        else
            this.timeFontstring:SetFont(defaultFont, LunaUF.db.profile.units["player"].auras.timertextbigsize, "OUTLINE")
        end
    ...
    elseif offEnchant then
        ...
        if this.cooldown:IsVisible() then
            CooldownFrame_SetTimer(this.cooldown, GetTime() - (LunaBuffDB[offEnchant] - offHandExpiration),
                LunaBuffDB[offEnchant], 1)
        end
    ...
    for i, button in ipairs(frame.auras.buffbuttons.buttons) do
        ...
        if texture then
            ...
            if timeleft and timeleft > 0 and buffName then
                ...
                CooldownFrame_SetTimer(button.cooldown, GetTime() - (LunaBuffDB[buffName] - timeleft),
                    LunaBuffDB[buffName], 1)
...
function Auras:UpdateLayout(frame)
    ...
    if config.debuffpos == "BOTTOM" then
        frame.auras.debuffbuttons:SetPoint("TOP", debuffanchor, "BOTTOM", 0,
            (config.buffs or config.weaponbuffs) and -3 or 0)
    elseif config.debuffpos == "TOP" then
        frame.auras.debuffbuttons:SetPoint("BOTTOM", debuffanchor, "TOP", 0,
            (config.buffs or config.weaponbuffs) and 3 or 0)
